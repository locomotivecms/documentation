...
| priority | Integer | used to position the file field in the back-office. Elements with the highest priority are displayed first. |
| resize | String | if the file is an image, apply an transformation. See the resize liquid filter for more examples. |
...
{% hint style="info" %}
**Put your google_analytics tag just before the closing BODY tag.**
