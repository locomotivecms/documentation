...
You can use any page as the template for another page. Inheritance consists in a ([Liquid tag](/liquid-api/tags#extends)), which must be written on the first line of a page underneath the metadata (`---`).
...
The principle of page's inheritance can be applied to every page. The index page is always the root page: you can either use `{% raw %}{% extends 'index' %}{% endraw %}` or `{% raw %}{% extends 'parent' %}{% endraw %}`.
