...
The available resource types are: content_entries, content_types, pages, site, snippets, and theme_assets.

## Options

| Name | Description |
| :--- | :--- |
| -r one, [--resources=one two three] | Only push the resource(s) passed in argument |
...
| -f filename, [--filter=filename] | Filter the resource(s) to be delopyed by its name plus extension. It can be used in conjunction with '-r' option to control exactly what is been pushed. |
