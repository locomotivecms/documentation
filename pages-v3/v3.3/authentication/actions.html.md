...
| auth_entry\[] | HTML form attributes of the user model. Require the value of [auth_id_field][auth_id_field], [auth_password_field][auth_password_field] and [auth_password_field][auth_password_field]_confirmation. Example: auth_entry[email], auth_entry[password] and auth_entry[password_confirmation][password_confirmation] |
...
If an "user" is found based on the auth*id and auth_password values, then the authenticated user will be available as a global liquid variable in all the further requests. The name of the variable is made from the slug of the content type: \`\`\`\{\{ current*\<singular version of the content type's slug> }}\`\`\`.
...
| Name | Description |
| :--- | :--- |
| auth_reset_\_instructions_sent | True if the email has been successfully sent |
| auth_wrong_\ | True if no user was found |
...
| auth_\_too_short | True if the password length is less than 6 characters |
...
<form action="{% path_to 'reset_password' %}" method="POST">
  ...
  {% if auth_invalid_token %}Your token is valid{% endif %}

  {% if auth_password_too_short %}Password too short!{% endif %}
