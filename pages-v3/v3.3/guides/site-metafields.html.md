...
Boolean values in metafields are not represented by `true` or `false`, but by `'1'` or `'0'`, which are both "truthy" in the Liquid templating language.  This means that any logical tests must be done by comparing the metafield's value against the string values of 1 or 0.  
...
{% code title="store_hours.liquid" %}
{% raw %}
```liquid
<!--  True only if \"Yes\" is set for this metafield. -->
{% if site.metafields.store_hours.saturday_effective == '1' %}
Saturday: 
{% endif %}
...
The first example returns true only if "Yes" is set for the `saturday_effective` metafield.  It will return false if "No" is set.  The second example will return true no matter what value is set for the metafield because true and false are represented by '1' and '0' respectively, which are both truthy in Liquid.
